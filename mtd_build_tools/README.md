...
* There is a valid `dw.json` file in the `mtd_build_tools` directory. See the `dw.json.example` in the `mtd_build_tools` directory  
...
there is not a `dw.json` in the `mtd_build_tools` directory)
...
Running `node mtd_build_tools/build --help` from the root of the project with generate help messaging regarding the scripts and allowed arguments.
