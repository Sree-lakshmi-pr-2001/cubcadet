...
/**
 * Object to store exported product IDs
 * @type {}
 */
var exportedProducts = [];

/**
 * Verify that productId is not exported yet
 * @param {string} productId - product ID value
 * @returns {boolean} - returns verification flag
 */
function verifyProductIdForExport(productId) {
    var goodToExport = true;
    for (var i = 0, l = exportedProducts.length; i < l; i++) {
        var productIds = exportedProducts[i];
...
/**
 * Save exported product ID
 * @param {string} productId - product ID
 */
function saveExportedProductId(productId) {
    // Verify that we have a least one array of product IDs
    // If not just add an empty one
    if (exportedProducts.length == 0) {
        var firstList = [];
        exportedProducts.push(firstList);
    }
    var latestIndex = exportedProducts.length - 1;
    // Check if the latest array list is not full
    if (exportedProducts[latestIndex].length >= 12000) {
        // Create a new array
        var newProductIdList = [];
        newProductIdList.push(productId);
        exportedProducts.push(newProductIdList);
    } else {
        exportedProducts[latestIndex].push(productId);
    }
}
 ...
 */
function process(productHit, parameters, stepExecution) {
    var skip = false;
    ...
    if (product.isVariant() && !skip) {
    	if (masterProduct.isOnline() && verifyProductIdForExport(masterProduct.ID)) {
            saveExportedProductId(masterProduct.ID);
    ...
    if (!skip && verifyProductIdForExport(product.ID)) {
        saveExportedProductId(product.ID);
