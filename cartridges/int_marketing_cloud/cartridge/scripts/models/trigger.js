...
function getTriggerDefinition(hookID, attributes) {
    Logger.debug('getTriggerDefinition ' + hookID);
...
function rebuildTriggerDefinition() {
    Logger.debug('rebuildTriggerDefinition');
...
function sendMessage() {
     ...
     */
     ...
     Logger.error('mtd - in trigger, this.storeId => ' + this.storeId);
    var msgSvc = mcServiceConfig.getMsgService('marketingcloud.rest.messaging.send', this.storeId);
    var message = this.message;
    Logger.error('SBD - SFMC payload ' + JSON.stringify(message));
...
Trigger.prototype = {
    ...
    send: function send(){
        Logger.error('mtd - sendMessage : ' + JSON.stringify(this));
        return sendMessage.apply(this, arguments);
    },

    setStoreId: function setStoreId(storeId){
