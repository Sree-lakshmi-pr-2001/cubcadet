...
var TaxModel = require('int_mtdservices/cartridge/scripts/models/Tax');
...
exports.calculate = function (basket) {
    ...
    // =====         CALCULATE TAX                   =====
    // ===================================================
    if (MTDUtil.VALUE.USE_BACKUP_TAX_TABLE) {
        HookMgr.callHook('dw.order.calculateTax', 'calculateTax', basket);
    }
    TaxModel.calculate(basket, false);
    ...
    while (productLineItems.hasNext()) {
        ...
        // handle bundle line items, but only if they're not a bonus
            ...
            if (productLineItem.product) {
                productLineItem.custom.bundledBasePrice = productLineItem.product.priceModel.price.value;
            }
...
exports.calculateTax = function(basket) {
    ...
    // we need to verify that we have a shipping address
    if (basket.defaultShipment.shippingAddress && basket.defaultShipment.shippingAddress.address1) {
