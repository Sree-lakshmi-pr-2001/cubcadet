 ...
 */
function addProductToCart(currentBasket, productId, quantity, childProducts, options, isARIRequest) {
    var availableToSell;
    ...
    if (product.bundle) {
        ...
        if (!canBeAdded && isARIRequest) {
            result.error = true;
            var errorMsg;
            var newQty = product.availabilityModel.inventoryRecord.ATS.value - qtyInCart;
