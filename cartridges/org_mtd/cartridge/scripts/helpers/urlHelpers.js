...
/**
 * Removes a query param from a URL
 *
 * @param {string} target - An URL endpoint with or without query params
 *     e.g. http://someresource or http://someresource?xyz=123
 * @param {string} queryParamToRemove - A query param to remove from the URL
 * @return {string} - Target URL with query param removed
 */
function removeQueryParam(target, queryParamToRemove) {
    if (!target || !queryParamToRemove) {
        return target;
    }
    ...
    delete targetQueryString[queryParamToRemove];
