...
function writeProductFeedItem(item, localeMap) {
	...
	switch(item.type) {
		...
		case 'Categories':
		    ...
		    writeElementCDATA('Name', category.displayName || category.ID);
		...
		case 'Products':
			...
			var productIsParts = MTDHelper.isPartsProduct(product);
			if(!productIsParts && product.online && product.searchable && (BV_Constants.EnableProductFamilies || !product.variant)) {
