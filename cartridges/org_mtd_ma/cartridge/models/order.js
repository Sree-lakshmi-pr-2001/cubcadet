...
var RESOURCES = {
    ...
    cardMaster: Resource.msg('msg.payment.type.credit.master', 'confirmation', null),
...
function getFirstProductLineItem(productLineItemsModel) {
    if (productLineItemsModel
            && productLineItemsModel.items[0]
            && productLineItemsModel.items[0].images
            && productLineItemsModel.items[0].images['line-item']
            && productLineItemsModel.items[0].images['line-item'].length > 0) {
        var firstItemImage = productLineItemsModel.items[0].images['line-item'][0];
        return {
...
function getCARBCompliantItemStatus(productLineItemsModel){
    var CARBCompliantItem = false;
    for each(var item in productLineItemsModel.items){
        if(item.CARBCompliantItem){
            CARBCompliantItem = true;
            break;
        }
    }
    return CARBCompliantItem;
}
...
function OrderModel(lineItemContainer, options) {
    ...
    if (!lineItemContainer) {
        ...
        var totalsModel = new TotalsModel(lineItemContainer, paymentModel);
        ...
        this.orderStatus = Object.hasOwnProperty.call(lineItemContainer, 'status')
            ? Resource.msg('label.order.status.' + lineItemContainer.status.displayValue, 'confirmation', null)
            : null;
        this.shippingStatus = Object.hasOwnProperty.call(lineItemContainer, 'shippingStatus')
            ? Resource.msg('label.order.shipping.status.' + lineItemContainer.shippingStatus.displayValue, 'confirmation', null)
            : null;
        ...
        if (modelConfig.numberOfLineItems === '*') {
            ...
            this.selectedShippingMethod = shippingModels[0].selectedShippingMethod || '';
            this.billing = billingModel;
        }

        this.prop65Ackowledged = 'prop65Acknowledged' in lineItemContainer.custom
        ? lineItemContainer.custom.prop65Acknowledged
        : null;

        this.notes = lineItemContainer.getNotes();
        this.CARBCompliantItemInCart = getCARBCompliantItemStatus(productLineItemsModel)
    }
}
