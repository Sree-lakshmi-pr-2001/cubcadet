...
function getShowMoreUrl(productSearch, httpParams) {
    var smPageSize = false;
    if (productSearch.category && productSearch.category.template === 'rendering/category/productLineBrowse') {
        smPageSize = true;
    }
    ...
    if (httpParams.scroll && httpParams.scroll < productSearch.count) {
        var defaultPageSize = smPageSize === false ? DEFAULT_PAGE_SIZE : 6;
        pageSize = Math.floor((httpParams.scroll / defaultPageSize) + 1) * defaultPageSize;
    } else {
        pageSize = smPageSize === false ? (parseInt(httpParams.sz, 10) || DEFAULT_PAGE_SIZE) : 6;
    }
 ...
 * @param {string} categoryTemplate - Search Result Category Template
 */
function ProductSearch(productSearch, httpParams, sortingRule, sortingOptions, rootCategory) {
    ...
    if (httpParams.scroll && httpParams.scroll < productSearch.count) {
        var defaultPageSize = smPageSize === false ? DEFAULT_PAGE_SIZE : 6;
        this.pageSize = Math.floor((httpParams.scroll / defaultPageSize) + 1) * defaultPageSize;
    } else {
        this.pageSize = smPageSize === false ? (parseInt(httpParams.sz, 10) || DEFAULT_PAGE_SIZE) : 6;
    }
