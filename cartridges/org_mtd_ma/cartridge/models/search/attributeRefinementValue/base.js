...
BaseAttributeValue.prototype = {
    ...
    getRefinementName: function (refinementName) {
    ...
    getUrl: function (productSearch, actionEndpoint, id, value, selected, selectable) {
        ...
        // Dynosite filters
        if (request.httpParameterMap.fitsOnModel.submitted) {
            var dynositeRegex = new RegExp(request.httpParameterMap.fitsOnModel.value + '(%20)?', 'g');
            url = url.replace(dynositeRegex, '');
        }
