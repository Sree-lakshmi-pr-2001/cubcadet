...
module.exports = function orderLineItem(product, apiProduct, options) {
    ...
    productDecorators.images(product, apiProduct, { types: ['line-item', 'mini-line-item'], quantity: 'single' });
    ...
    productLineItemDecorators.trackingNumber(product, options.lineItem.custom.pos_trackingNumbers);

    var isOrderModel = true;
    productDecorators.setExtendedWarranty(product, apiProduct, options.variationModel, isOrderModel);
    return product;
};
