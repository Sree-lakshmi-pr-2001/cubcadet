    ...
    collections.forEach(collection, function (item) {
        if (!item.basedOnCoupon) {
            result[item.UUID] = {
                ...
                type: 'promotion',
                callOutMsg: (item.promotion) ? item.promotion.calloutMsg : ''
            };
        }
    });
 ...
 */
function totals(lineItemContainer,paymentModel) {
    if (lineItemContainer) {
        ...
        if (this.totalShippingCost === '-') {
            ...
            if(lineItemContainer.paymentInstrument && lineItemContainer.paymentInstrument.paymentMethod == 'TD_FINANCE' && paymentModel &&  paymentModel.selectedPaymentInstruments[0]){
                var tdFinaceTotal = paymentModel.selectedPaymentInstruments[0]
                this.grandTotal = tdFinaceTotal.formattedAmount;
                session.custom.tdFinanceAmount = paymentModel.selectedPaymentInstruments[0].tdAmount
            }else{
