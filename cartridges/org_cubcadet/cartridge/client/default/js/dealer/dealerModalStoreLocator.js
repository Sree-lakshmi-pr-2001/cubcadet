/* globals google */
'use strict';
var base = require('org_ma/storeLocator/storeLocator');
...
/**
 * Update Dealer Events
 */
function updateDealerEvents() {
    $('.store-details').find('button[data-event-type="selectDealer"]').on('click', function () {
        $('#dealerSelectorModal').spinner().start();
        var storeId = $(this).closest('.store-details').data('storeId');
        var postalCodeValue = $('#dealerSelectorModal .store-postal-code-input').val();

        var url = $(this).data('action');

        if (url.includes('en_CA') || url.includes('fr_CA')) {
            // Changing zipcode to uppercase
            postalCodeValue = postalCodeValue.toUpperCase();
            // Removing dash from zip code
            postalCodeValue = postalCodeValue.replace('-', ' ');

            if (postalCodeValue.charAt(3) !== ' ') {
                postalCodeValue = postalCodeValue.replace(/.{1,2}(?=(.{3})+$)/g, '$& ');
            }
        }

        var params = {
            selectedDealer: storeId,
            postalCode: (postalCodeValue && postalCodeValue.length > 0) ? postalCodeValue : ''
        };
...
function search(element) {
    ...
    if (url.includes('en_CA') || url.includes('fr_CA')) {
        // Changing zip code to uppercase
        dataObj.address = dataObj.address.toUpperCase();
        // Removing dash from zip code
        ...
        success: function (geoData) {
            if (geoData.status === 'OK') {
                ...
                if ($form.hasClass('service-locator-form')) {
                    ...
                    var requestData = payload + '&lat=' + lat + '&long=' + long;
                    var productId = $('#dealerSelectorModal').attr('data-product-id');
                    var productQuantity = $('#dealerSelectorModal').attr('data-product-quantity');
                    var shippingMethod = $('#dealerSelectorModal').attr('data-shipping-method');
                    var eventType = $('#dealerSelectorModal').attr('data-event-type');
                    var postalCodeValue = $('#dealerSelectorModal .store-postal-code-input').val();
                    ...
                    if (productId && productId.length > 0) {
                        requestData = requestData + '&productId=' + productId;
                    }
                    if (productQuantity && productQuantity.length > 0) {
                        requestData = requestData + '&productQuantity=' + productQuantity;
                    }
                    if (eventType && eventType.length > 0) {
                        requestData = requestData + '&eventType=' + eventType;
                    }
                    if (postalCodeValue && postalCodeValue.length > 0) {
                        requestData = requestData + '&postalCode=' + postalCodeValue;
                    }
                    if (shippingMethod && shippingMethod.length > 0) {
                        requestData = requestData + '&shippingMethod=' + shippingMethod;
                    }
...
var exportBase = $.extend({}, base, {
    ...
    search: function () {
        ...
        $('.dealer-locator [name="address"]').change(function (e) {
            e.preventDefault();
        ...
        $('#dealerSelectorModal').on('hide.bs.modal', function (e) {
            // Don't allow to hide modal while searching is in progress
            if (isSearching) {
                e.preventDefault();
            }
        });
