...
server.get(
    ...
    function (req, res, next) {
        ...
        if (!order
            ...
            || order.customer.ID !== session.customer.ID
        ) {
            res.render('/error', {
...
server.get(
    ...
    function (req, res, next) {
        ...
        if (req.querystring.trackOrderEmail
            ...
            order = OrderMgr.getOrder(req.querystring.trackOrderNumber.toUpperCase());
        } else {
            validForm = false;
        }
...
//          @TODO SFRA Issue 74 - END
            ...
            var trackOrderPostal = req.querystring.trackOrderPostal.replace(' ', '').toLowerCase();
            var billingPostal = orderModel.billing.billingAddress.address.postalCode.replace(' ', '').toLowerCase();
            var billingPostalArr = billingPostal.split('-');
            var billingPostalPartial = billingPostalArr[0];
            if (trackOrderPostal !== billingPostal
                    && billingPostalPartial !== trackOrderPostal) {
                validForm = false;
            }
...
server.get(
    ...
    function (req, res, next) {
        ...
        res.render('account/order/history', {
            ...
            orderFilter: req.querystring.orderFilter || '6',
        ...
        if (passwordForm.valid) {
            ...
            this.on('route:BeforeComplete', function (req, res) { // eslint-disable-line no-shadow
                ...
                try {
                    ...
                    errorObj.error = true;
                    if ('causeName' in e && e.causeName === 'PasswordFormatException') {
                        errorObj.errorMessage = Resource.msg('error.message.password.constraints.not.matched', 'forms', null);
                    } else {
