...
function appendBVData(req, res) {
	...
	if(BVHelper.isRREnabled() || BVHelper.isQAEnabled()) {
        ...
        var aggregateRating = seoReviews.getAggregateRating();

        // removing schema tags to prevent SEO errors with duplicate code
        // mobile ratings contain the schema in source, desktop ratings will have it removed
        var itemProp = 'itemprop="aggregateRating"';
        var itemScope = 'itemscope';
        var itemType = 'itemtype="http://schema.org/AggregateRating"';
        var modifiedAggregateRating = aggregateRating.replace(itemProp, '').replace(itemScope, '').replace(itemType, '');
		...
		viewData.bvDisplay = {
			rr: {
				...
				seo: {
                    ...
                    modifiedAggregateRating: modifiedAggregateRating,
